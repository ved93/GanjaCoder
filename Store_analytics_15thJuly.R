...
#load libraries
library(RODBC)
channel<- odbcConnect("ForecastDemo", uid="ved", pwd="pwd")

 

UI_table<-sqlQuery(channel, "select * from [MyIpParameters_tb]")

UI_table<-UI_table[UI_table$Active==1,]

ID<-UI_table$Id

for(i in 1:length(unique(UI_table$DataSource))) 
{
  UI_table<-sqlQuery(channel, "select * from [MyIpParameters_tb]")
  
  UI_table<-UI_table[UI_table$Active==1,]
  
  m<-UI_table$DataSource[i]
  
  UI_table<-UI_table[UI_table$DataSource == m,]
  
  for (j in 1:length(unique(UI_table$Dimension))) {
    
    p<-UI_table$Dimension[j]
    
    
    
    if(UI_table$Values[j] == " ")
      
    {
      ...
      data<-sqlQuery(channel, paste("SELECT * FROM  [",m,"]  ", sep= ""))
      
      data$CreateDate <- as.POSIXlt(data$CreateDate)
      
      
      if((((Store_data$CreateDate[i+1] - Store_data$CreateDate[i]) ==1) &&  (data$CreateDate[i+1] - data$CreateDate[i]) ==7) )
      ...
      { 
        ...
        for(i in length(data$CreateDate):1 )
        {
          ...
          Store_data$t<- ifelse(Store_data$CreateDate <= data$CreateDate[i] , b ,Store_data$t) 
        ...
        colnames(Store_data)[ncol(Store_data)]<-t
      ...
      if((((Store_data$CreateDate[i+1] - Store_data$CreateDate[i]) ==1) &&  (data$CreateDate[i+1] - data$CreateDate[i]) ==1) )
      {
    ...
    if(UI_table$Values[j] != " ")
    {
      ...
      Store_data<-sqlQuery(channel, paste("SELECT * FROM  [",m,"] where [",p,"] = '",q,"' ", sep= ""))
      
      Store_data$CreateDate <- as.POSIXlt(Store_data$CreateDate)
      
      new_features<-ncol(Store_data)
...
#m,layers,neurons
  ...
  #model_name<-"modelX_GasPrices_tb--X_SnowFall_tb"
